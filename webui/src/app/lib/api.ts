...
export async function createCluster(
  ...
  try {
    const { data: responseData } = await axios.post(`${apiHost}/namespaces/${namespace}/clusters`, {
      name,
      nodes,
      replicas,
      password,
...
export async function fetchClusters(namespace: string): Promise<string[]> {
  try {
    const { data: responseData } = await axios.get(`${apiHost}/namespaces/${namespace}/clusters`);
